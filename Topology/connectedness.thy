...
lemma conn_prop5: assumes mono: \<open>MONO \<C>\<close> and cl2: \<open>Cl_2 \<C>\<close> shows
 \<open>(\<forall>p q. X p \<and> X q \<and> (\<exists>S. S \<^bold>\<preceq> X \<and> Connected[\<C>] S \<and> S p \<and> S q)) \<longrightarrow> Connected[\<C>] X\<close>
proof -
{ assume premise: \<open>\<forall>p q. X p \<and> X q \<and> (\<exists>S. S \<^bold>\<preceq> X \<and> Connected[\<C>] S \<and> S p \<and> S q)\<close>
  have \<open>Connected[\<C>] X\<close> proof
    ...
    from nonempty have \<open>X ?p \<and> X ?q\<close> 
      by (simp add: premise)
    hence \<open>\<exists>S. S \<^bold>\<preceq> X \<and> Connected[\<C>] S \<and> S ?p \<and> S ?q\<close> 
      by (simp add: premise)
    then obtain S where aux: \<open>S \<^bold>\<preceq> X \<and> Connected[\<C>] S \<and> S ?p \<and> S ?q\<close> ..
    from aux nonempty have \<open>\<not>Disj S A \<and> \<not>Disj S B\<close> 
      by (metis Disj_def someI)
    moreover from aux mono sepXAB have \<open>S \<^bold>\<preceq> A \<or> S \<^bold>\<preceq> B\<close> 
      using conn_prop3 by blast
