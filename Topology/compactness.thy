...
(**It can be shown that FIP is equivalent to the following "binary intersection property" (BIP)*)
definition "BIP S \<equiv> \<forall>A B. S A \<and> S B \<longrightarrow> \<not>(A \<^bold>\<and> B \<^bold>\<approx> \<^bold>\<bottom>)"
(**Both definitions are equivalent*)
lemma FBIP_equiv: "BIP S = FIP S" sorry (*Exercise: prove this*)
...
(**and, similarly, the FUP is equivalent to its binary counterpart (BUP)*)
definition "BUP S \<equiv> \<exists>A B. S A \<and> S B \<and> (A \<^bold>\<or> B \<^bold>\<approx> \<^bold>\<top>)"
(**Both definitions are equivalent*)
lemma FBUP_equiv: "BUP S = FUP S" sorry (*Exercise: prove this*)
...
lemma BIUP_dual2: "(\<not>BUP S) = BIP S\<^sup>-" using BA_deMorgan1 BA_deMorgan2 unfolding BIP_def BUP_def by (smt (z3) BA_dn L13 L14 L2 L4 L5 L9 compl_def dom_compl_def setequ_char setequ_def setequ_equ subset_def)

lemma FIUP_dual1: "(\<not>FIP S) = FUP S\<^sup>-" using BIUP_dual1 FBIP_equiv FBUP_equiv by blast
lemma FIUP_dual2: "(\<not>FUP S) = FIP S\<^sup>-" using BIUP_dual2 FBIP_equiv FBUP_equiv by blast
...
lemma "compact\<^sup>c\<^sup>l \<C> = compact\<^sup>o\<^sup>p \<C>" 
  unfolding compact_cl_def compact_op_def by (smt (verit) ClOpdual FIUP_dual1 FIUP_dual2 OpCldual bottom_def compl_def dom_compl_def iDM_a iDM_b setequ_char top_def) 
