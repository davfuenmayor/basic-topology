...
lemma FIUP_dual1: "(\<not>FIP S) = FUP S\<^sup>-" oops (*Exercise: prove this (under minimal conditions)*)
lemma FIUP_dual2: "(\<not>FUP S) = FIP S\<^sup>-" oops (*Exercise: prove this (under minimal conditions)*)
...
lemma "compact\<^sup>c\<^sup>l \<C> = compact\<^sup>o\<^sup>p \<C>" oops (*Exercise: prove this (under minimal conditions)*)
